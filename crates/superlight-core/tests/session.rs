...
use superlight_core::{
    hidpp::{self, ProtocolError, ScrollMode, SmartShift},
...
fn reply(device: u8, feature: u8, function: u8, params: &[u8]) -> Vec<u8> {
    hidpp::encode(device, feature, function, params)
        .unwrap()
        .to_vec()
}
...
fn forwards_button_release_while_waiting_for_a_setting_reply() {
    ...
    session
        .request(7, 2, &[2, 25, 0], &mut |report| events.push(report))
        .unwrap();
...
fn unrelated_receiver_replies_and_notifications_do_not_escape_the_slot() {
    ...
    notification[3] = 0;
    let (mut session, _) = setup(vec![
        reply(2, 7, 2, &[9]),
        notification,
        reply(255, 7, 2, &[3]),
    ]);
    let result = session
        .request(7, 2, &[], &mut |_| panic!("Wrong receiver slot"))
        .unwrap();
...
fn rawxy_rejection_falls_back_to_button_only_diversion() {
    ...
    let diverted = session
        .divert_gesture(&[195, 215], &mut |_| {})
        .unwrap()
        .unwrap();
...
fn destructor_restores_extra_buttons_before_gesture() {
    ...
    session
        ...
        .unwrap();
    session
        .divert_extra(hidpp::DPI_SWITCH_CID, &mut |_| {})
        .unwrap();
    drop(session);
    let state = state.borrow();
...
fn enhanced_smart_shift_uses_distinct_function_ids() {
    ...
    session
        .set_smart_shift(SmartShift::default(), &mut |_| {})
        .unwrap();
    assert_eq!(
        session.read_smart_shift(&mut |_| {}).unwrap(),
        SmartShift {
...
fn device_names_are_length_bounded_and_reassembled_from_chunks() {
    ...
    let (mut session, state) = setup(vec![
        reply(255, 9, 0, &[name.len() as u8]),
        reply(255, 9, 1, &name[..16]),
        reply(255, 9, 1, &name[16..]),
    ]);
    session.features.name = Some(9);
    assert_eq!(
        session.read_name(&mut |_| {}).unwrap(),
...
fn missing_features_never_send_writes_to_root() {
    ...
    assert_eq!(
        session.set_smart_shift(SmartShift::default(), &mut |_| {}),
