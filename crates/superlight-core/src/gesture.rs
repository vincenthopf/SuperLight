...
impl Direction {
    pub const fn mapping_index(self) -> usize {
        match self { Self::Left => 8, Self::Right => 9, Self::Up => 10, Self::Down => 11 }
    }
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum Source { Hid, Native }
...
impl Default for Options {
    fn default() -> Self {
        Self { enabled: false, threshold: 50.0, deadzone: 40.0, timeout_ms: 3000, cooldown_ms: 500, prefer_hid: cfg!(target_os = "macos") }
    }
}
...
impl Gesture {
    pub fn new(options: Options) -> Self { Self { options, ..Self::default() } }

    pub fn held(&self) -> bool { self.held }
    pub fn source(&self) -> Option<Source> { self.source }
    ...
    pub fn press(&mut self, now_ms: u64) {
        ...
        self.tracking = false;
        if self.options.enabled && now_ms >= self.cooldown_until_ms { self.reset_segment(now_ms); }
    }
    ...
    pub fn movement(&mut self, x: f64, y: f64, source: Source, now_ms: u64) -> Option<Direction> {
        if !self.held || !self.options.enabled || !x.is_finite() || !y.is_finite() || now_ms < self.cooldown_until_ms { return None; }
        let promote = self.options.prefer_hid && source == Source::Hid && self.source == Some(Source::Native);
        if !self.tracking || now_ms.saturating_sub(self.last_move_ms) > self.options.timeout_ms.max(250) || promote { self.reset_segment(now_ms); }
        if self.source.is_some_and(|existing| existing != source) { return None; }
        self.source = Some(source);
        self.x = (self.x + x).clamp(-1_000_000.0, 1_000_000.0);
        self.y = (self.y + y).clamp(-1_000_000.0, 1_000_000.0);
        self.last_move_ms = now_ms;
        let direction = detect(self.x, self.y, self.options.threshold, self.options.deadzone)?;
        self.triggered = true;
...
pub fn detect(x: f64, y: f64, threshold: f64, deadzone: f64) -> Option<Direction> {
    ...
    if dominant < threshold.max(5.0) { return None; }
    let cross_limit = deadzone.max(0.0).max(dominant * 0.35);
    if abs_x > abs_y {
        if abs_y > cross_limit { return None; }
        Some(if x > 0.0 { Direction::Right } else { Direction::Left })
    } else {
        if abs_x > cross_limit { return None; }
...
impl WheelAccumulator {
    pub fn step(&mut self, delta: f64, threshold: f64, volume: bool, now_ms: u64) -> bool {
        if !delta.is_finite() || !threshold.is_finite() { return false; }
        let cooldown_ms = if volume { 60 } else { 350 };
        if self.last_fire_ms.is_some_and(|last| now_ms.saturating_sub(last) < cooldown_ms) {
            self.accumulated = 0.0;
            return false;
        }
        self.accumulated += delta.abs().min(1.0);
        if self.accumulated < threshold.max(0.1) { return false; }
        self.accumulated = 0.0;
...
mod tests {
    ...
    fn click_is_only_emitted_on_an_untriggered_release() {
        let mut gesture = Gesture::new(Options { enabled: true, ..Options::default() });
        assert!(!gesture.release());
        gesture.press(0);
        assert!(gesture.release());
        gesture.press(10);
        assert_eq!(gesture.movement(50.0, 0.0, Source::Hid, 11), Some(Direction::Right));
        assert!(!gesture.release());
    }

    #[test]
    fn mac_promotes_hid_after_tiny_native_starter() {
        let mut gesture = Gesture::new(Options { enabled: true, prefer_hid: true, ..Options::default() });
        gesture.press(0);
        assert_eq!(gesture.movement(2.0, 0.0, Source::Native, 1), None);
        assert_eq!(gesture.movement(-50.0, 0.0, Source::Hid, 2), Some(Direction::Left));
        assert!(!gesture.release());
    }
    ...
    fn no_double_counting_and_timeout_starts_new_segment() {
        ...
        assert_eq!(gesture.movement(20.0, 0.0, Source::Hid, 4001), Some(Direction::Right));
    }
