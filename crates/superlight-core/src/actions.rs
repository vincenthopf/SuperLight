...
impl Chord {
    pub fn parse(combo: &str, platform: Platform) -> Result<Self, String> {
        let mut chord = Self { codes: [0; 8], len: 0, phased: false };
        for name in combo.split('+').map(str::trim).filter(|name| !name.is_empty()) {
            if usize::from(chord.len) == chord.codes.len() {
                return Err("Shortcuts accept at most eight keys".into());
            }
            chord.codes[usize::from(chord.len)] = key_code(name, platform)
                .ok_or_else(|| format!("Unknown shortcut key: {name}"))?;
            chord.len += 1;
        }
        if chord.len == 0 {
...
pub const ACTIONS: &[(&str, &str)] = &[
    ...
    ("undo", "Undo"), ("select_all", "Select all"), ("save", "Save"),
    ("next_tab", "Next tab"), ("prev_tab", "Previous tab"),
    ("close_tab", "Close tab"), ("new_tab", "New tab"), ("find", "Find"),
    ("win_d", "Desktop overview"), ("task_view", "Task view"),
    ("mission_control", "Mission Control"), ("app_expose", "Application overview"),
    ("space_left", "Previous desktop"), ("space_right", "Next desktop"),
    ("show_desktop", "Show desktop"), ("launchpad", "Applications"),
    ("volume_up", "Volume up"), ("volume_down", "Volume down"),
    ("volume_mute", "Mute"), ("play_pause", "Play / pause"),
    ("next_track", "Next track"), ("prev_track", "Previous track"),
    ("page_up", "Page up"), ("page_down", "Page down"),
    ("home", "Home"), ("end", "End"),
    ("switch_scroll_mode", "Ratchet / free spin"),
    ("toggle_smart_shift", "Toggle SmartShift"), ("cycle_dpi", "Cycle DPI presets"),
...
impl Action {
    pub fn parse(id: &str, platform: Platform) -> Result<Self, String> {
        if id == "none" || id.is_empty() { return Ok(Self::None); }
        ...
        let chord = match id {
            ...
            "alt_tab" => if mac { "super+tab" } else { "alt+tab" },
            "alt_shift_tab" => if mac { "super+shift+tab" } else { "alt+shift+tab" },
            "browser_back" => if mac { "super+[" } else { "alt+left" },
            "browser_forward" => if mac { "super+]" } else { "alt+right" },
            "copy" => if mac { "super+c" } else { "ctrl+c" },
            "paste" => if mac { "super+v" } else { "ctrl+v" },
            "cut" => if mac { "super+x" } else { "ctrl+x" },
            "undo" => if mac { "super+z" } else { "ctrl+z" },
            "select_all" => if mac { "super+a" } else { "ctrl+a" },
            "save" => if mac { "super+s" } else { "ctrl+s" },
            "next_tab" => if mac { "super+shift+]" } else { "ctrl+tab" },
            "prev_tab" => if mac { "super+shift+[" } else { "ctrl+shift+tab" },
            "close_tab" => if mac { "super+w" } else { "ctrl+w" },
            "new_tab" => if mac { "super+t" } else { "ctrl+t" },
            "find" => if mac { "super+f" } else { "ctrl+f" },
            "win_d" => if mac { "ctrl+up" } else { "super+d" },
            "task_view" => if mac { "ctrl+up" } else { "super+tab" },
    ...
    pub fn is_device(self) -> bool {
        matches!(self, Self::ToggleSmartShift | Self::SwitchScrollMode | Self::CycleDpi)
    }
}
...
pub fn key_code(name: &str, platform: Platform) -> Option<u16> {
    ...
    let column = match platform { Platform::MacOs => 0, Platform::Windows => 1, Platform::Linux => 2 };
    let codes = match name {
        "ctrl" => [0x3b, 0x11, 29], "shift" => [0x38, 0x10, 42],
        "alt" => [0x3a, 0x12, 56], "super" => [0x37, 0x5b, 125],
        "tab" => [0x30, 9, 15], "space" => [0x31, 32, 57],
        "enter" => [0x24, 13, 28], "esc" => [0x35, 27, 1],
        "backspace" => [0x33, 8, 14], "delete" => [0x75, 46, 111],
        "left" => [0x7b, 37, 105], "right" => [0x7c, 39, 106],
        "up" => [0x7e, 38, 103], "down" => [0x7d, 40, 108],
        "pageup" => [0x74, 33, 104], "pagedown" => [0x79, 34, 109],
        "home" => [0x73, 36, 102], "end" => [0x77, 35, 107],
        "[" => [0x21, 0xdb, 26], "]" => [0x1e, 0xdd, 27],
        ...
        _ => {
            if name.len() == 1 {
                ...
                if byte.is_ascii_lowercase() {
                    ...
                    let mac = [0, 11, 8, 2, 14, 3, 5, 4, 34, 38, 40, 37, 46, 45, 31, 35, 12, 15, 1, 17, 32, 9, 13, 7, 16, 6];
                    let linux = [30, 48, 46, 32, 18, 33, 34, 35, 23, 36, 37, 38, 50, 49, 24, 25, 16, 19, 31, 20, 22, 47, 17, 45, 21, 44];
                    return Some([mac[index], u16::from(byte.to_ascii_uppercase()), linux[index]][column]);
                }
                if byte.is_ascii_digit() {
                    let index = usize::from(byte - b'0');
                    return Some([[29, 18, 19, 20, 21, 23, 22, 26, 28, 25][index], u16::from(byte), [11, 2, 3, 4, 5, 6, 7, 8, 9, 10][index]][column]);
                }
            }
            if let Some(function) = name.strip_prefix('f').and_then(|value| value.parse::<usize>().ok()).filter(|value| (1..=12).contains(value)) {
                let index = function - 1;
                return Some([[122, 120, 99, 118, 96, 97, 98, 100, 101, 109, 103, 111][index], (0x70 + index) as u16, [59, 60, 61, 62, 63, 64, 65, 66, 67, 68, 87, 88][index]][column]);
            }
            return None;
        }
    };
...
mod tests {
    ...
    fn every_builtin_compiles_on_all_platforms() {
        for platform in [Platform::MacOs, Platform::Windows, Platform::Linux] {
            for (id, _) in ACTIONS { assert!(Action::parse(id, platform).is_ok(), "{id}"); }
        }
    }
    ...
    fn aliases_match_canonical_keys_on_all_platforms() {
        for platform in [Platform::MacOs, Platform::Windows, Platform::Linux] {
            for (alias, canonical) in [("command", "super"), ("win", "super"), ("opt", "alt"), ("control", "ctrl"), ("return", "enter"), ("escape", "esc")] {
    ...
    fn mac_digits_are_hardware_codes_not_ascii() {
        assert_eq!(Chord::parse("cmd+shift+3", Platform::MacOs).unwrap().keys(), [55, 56, 20]);
        assert_eq!(Chord::parse("ctrl+0", Platform::Windows).unwrap().keys(), [17, 48]);
    }
    ...
    fn chromium_navigation_keeps_phased_left_alt() {
        let Action::Chord(chord) = Action::parse("browser_back", Platform::Windows).unwrap() else { panic!() };
        assert!(chord.phased);
        assert_eq!(chord.keys(), [164, 37]);
        let Action::Chord(chord) = Action::parse("browser_back", Platform::MacOs).unwrap() else { panic!() };
        assert!(!chord.phased);
