...
impl From<ProtocolError> for Error { fn from(error: ProtocolError) -> Self { Self::Protocol(error) } }
...
impl Report {
    pub fn from_message(message: hidpp::Message<'_>) -> Self {
        ...
        Self { device: message.device, feature: message.feature, function: message.function, software: message.software, parameters, len: len as u8 }
    }

    pub fn params(&self) -> &[u8] { &self.parameters[..usize::from(self.len)] }
}
...
impl<T: Transport> Session<T> {
    pub fn new(transport: T, device_index: u8) -> Self {
        Self {
            transport: Some(transport), device_index, features: Features::default(),
            timeout: Duration::from_secs(2), timeouts: 0, diverts: [None; 3],
        }
    }

    pub fn retarget(&mut self, device_index: u8) -> Result<(), Error> {
        if self.diverts.iter().any(Option::is_some) { return Err(ProtocolError::Malformed.into()); }
    ...
    pub fn needs_reconnect(&self) -> bool { self.timeouts >= 3 }
    pub fn diverts(&self) -> &[Option<Divert>; 3] { &self.diverts }

    fn transport(&mut self) -> Result<&mut T, Error> {
        self.transport.as_mut().ok_or_else(|| Error::Transport("HID transport is closed".into()))
    }

    pub fn request(&mut self, feature: u8, function: u8, parameters: &[u8], notify: &mut impl FnMut(Report)) -> Result<Report, Error> {
        self.request_with_timeout(feature, function, parameters, self.timeout, notify)
    }

    pub fn request_with_timeout(&mut self, feature: u8, function: u8, parameters: &[u8], timeout: Duration, notify: &mut impl FnMut(Report)) -> Result<Report, Error> {
        ...
        while let Some(remaining) = timeout.checked_sub(started.elapsed()) {
            ...
            let len = self.transport()?.read_wait(&mut buffer, remaining.min(Duration::from_millis(250)))?;
            if len > buffer.len() { return Err(ProtocolError::Malformed.into()); }
            let Some(message) = hidpp::parse(&buffer[..len]) else { continue; };
            match hidpp::match_response(message, self.device_index, feature, function) {
                ...
                ResponseMatch::Unrelated => {
                    if message.device == self.device_index && message.software == 0 { notify(Report::from_message(message)); }
                }
            }
        }
    ...
    pub fn read_notification(&mut self, timeout: Duration, notify: &mut impl FnMut(Report)) -> Result<bool, Error> {
        let mut buffer = [0; 64];
        let len = self.transport()?.read_wait(&mut buffer, timeout)?;
        if len > buffer.len() { return Err(ProtocolError::Malformed.into()); }
        if let Some(message) = hidpp::parse(&buffer[..len])
            && message.device == self.device_index && message.software == 0 {
            notify(Report::from_message(message));
        }
        Ok(len != 0)
    }

    pub fn feature(&mut self, id: u16, notify: &mut impl FnMut(Report)) -> Result<Option<u8>, Error> {
    ...
    pub fn discover_controls(&mut self, notify: &mut impl FnMut(Report)) -> Result<Vec<Control>, Error> {
        let feature = self.features.reprog.ok_or(Error::Unsupported)?;
        let response = self.request(feature, 0, &[], notify)?;
        let count = usize::from(response.params().first().copied().unwrap_or(0)).min(hidpp::MAX_CONTROLS);
        let mut controls = Vec::with_capacity(count);
        let mut failures = 0;
        for index in 0..count {
            let response = match self.request_with_timeout(feature, 1, &[index as u8], Duration::from_millis(500), notify) {
            ...
            failures = 0;
            let Some(mut control) = Control::decode(index as u8, response.params()) else { continue; };
            if let Ok(reporting) = self.request(feature, 2, &control.cid.to_be_bytes(), notify) { control.apply_reporting(reporting.params()); }
    ...
    pub fn read_name(&mut self, notify: &mut impl FnMut(Report)) -> Result<String, Error> {
        ...
        Ok(String::from_utf8_lossy(&bytes).trim_matches('\0').trim().to_owned())
    }

    pub fn discover_optional_features(&mut self, notify: &mut impl FnMut(Report)) -> Result<(), Error> {
        ...
        if self.features.smart_shift.is_none() { self.features.smart_shift = self.feature(hidpp::SMART_SHIFT, notify)?; }
        ...
        if self.features.battery.is_none() { self.features.battery = self.feature(hidpp::BATTERY_STATUS, notify)?; }
        Ok(())
    }

    fn set_reporting(&mut self, cid: u16, flags: u8, notify: &mut impl FnMut(Report)) -> Result<(), Error> {
        let feature = self.features.reprog.ok_or(Error::Unsupported)?;
    ...
    pub fn divert_gesture(&mut self, candidates: &[u16], notify: &mut impl FnMut(Report)) -> Result<Option<Divert>, Error> {
        if self.diverts[0].is_some() { return Ok(self.diverts[0]); }
    ...
    pub fn divert_extra(&mut self, cid: u16, notify: &mut impl FnMut(Report)) -> Result<(), Error> {
        let slot = match cid { hidpp::MODE_SHIFT_CID => 1, hidpp::DPI_SWITCH_CID => 2, _ => return Err(Error::Unsupported) };
        if self.diverts[slot].is_some() { return Ok(()); }
    ...
    pub fn shutdown(&mut self) {
        for slot in [1, 2, 0] { let _ = self.undivert_slot(slot); }
    }
    ...
    pub fn read_dpi(&mut self, notify: &mut impl FnMut(Report)) -> Result<u16, Error> {
        ...
        if params.len() < 3 { return Err(ProtocolError::Malformed.into()); }
        Ok(u16::from_be_bytes([params[1], params[2]]))
    }

    pub fn set_smart_shift(&mut self, state: SmartShift, notify: &mut impl FnMut(Report)) -> Result<(), Error> {
        let feature = self.features.smart_shift.ok_or(Error::Unsupported)?;
        let function = if self.features.enhanced_smart_shift { 2 } else { 1 };
        self.request(feature, function, &state.wire_parameters(), notify)?;
        Ok(())
    }

    pub fn read_smart_shift(&mut self, notify: &mut impl FnMut(Report)) -> Result<SmartShift, Error> {
        let feature = self.features.smart_shift.ok_or(Error::Unsupported)?;
        let function = u8::from(self.features.enhanced_smart_shift);
        let response = self.request(feature, function, &[], notify)?;
        let params = response.params();
        if params.len() < 2 { return Err(ProtocolError::Malformed.into()); }
    ...
    pub fn read_battery(&mut self, notify: &mut impl FnMut(Report)) -> Result<u8, Error> {
        ...
        let response = self.request(feature, function, &[], notify)?;
        response.params().first().copied().filter(|percent| *percent <= 100).ok_or(ProtocolError::Malformed.into())
    }
}
