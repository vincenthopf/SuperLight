...
use superlight_ipc::{DeviceStatus, PROTOCOL_VERSION, Paths, Request, Response, Snapshot};
...
impl Client {
    fn new(context: egui::Context) -> std::io::Result<Self> {
        ...
        thread::Builder::new()
            .name("superlight-ui-control".into())
            .spawn(move || {
                while let Ok(job) = work.recv() {
                    let result = Paths::discover()
                        .and_then(|paths| superlight_ipc::call(&paths, &job.request))
...
impl Settings {
    fn new(
        ...
        for (name, bytes) in [
            ...
            textures.push(context.egui_ctx.load_texture(
                name,
        ...
        Ok(Self {
            ...
            allow_close: false,
            smoke_frames: smoke.then_some(0),
        })
    }
    ...
    fn sync_custom(&mut self) {
        ...
        self.custom = self.editor.draft["profiles"][&self.editor.selected_profile]["mappings"][key]
            .as_str()
    ...
    fn receive(&mut self) {
        let reply = self
            .client
            .as_ref()
            .and_then(|client| client.replies.try_recv().ok());
        ...
        match reply.result {
            Ok(response) if response.protocol != PROTOCOL_VERSION => {
                ...
                self.message =
                    Some("The service and settings window use different protocol versions".into());
            }
                ...
                if let Some(state) = response.snapshot {
                    ...
                    if replace {
                        if let Some(device) = &state.device {
                            self.layout = if device.layout_key.contains("anywhere") {
                                1
                            } else if device.layout_key.contains("vertical") {
                                2
                            } else {
                                0
                            };
                    ...
                    if reply.save {
                        self.editor.saved(state);
                    } else {
                        self.editor.receive(state);
                    }
                    self.connected = true;
                    if replace {
                        self.sync_fields();
                    ...
                    if reply.save {
                        self.message =
                            Some("Settings saved. Hardware changes may still be pending.".into());
                    }
                } else {
                    self.message = Some("The service returned no configuration".into());
                }
            }
    ...
    fn top(&mut self, ui: &mut egui::Ui) {
        ui.horizontal(|ui| {
            ...
            if self.demo {
                ui.label("Preview only. No hardware connection.");
            } else if self.busy {
                ui.spinner();
            } else if self.connected {
                ui.label("Connected");
            } else {
                ui.label("Service not connected");
            }
        });
        if let Some(state) = &self.editor.latest {
            ...
            ui.label(status);
            if !self.demo && !state.permissions.description.is_empty() {
                ui.label(&state.permissions.description);
            }
        }
        ui.horizontal(|ui| {
            for (index, name) in ["Mouse", "Point and scroll", "Profiles", "Diagnostics"]
                .into_iter()
                .enumerate()
            {
    ...
    fn profile_selector(&mut self, ui: &mut egui::Ui) {
        ...
        let label = profiles
            .iter()
            .find(|(key, _)| key == &old)
            .map_or(old.as_str(), |(_, label)| label.as_str());
        egui::ComboBox::from_id_salt("profile")
            .selected_text(label)
            .show_ui(ui, |ui| {
                for (key, label) in profiles {
                    ui.selectable_value(&mut self.editor.selected_profile, key, label);
                }
            });
        if old != self.editor.selected_profile {
            self.sync_fields();
    ...
    fn mouse_view(&mut self, ui: &mut egui::Ui) {
        ...
        ui.horizontal(|ui| {
            ...
            egui::ComboBox::from_id_salt("layout")
                .selected_text(["MX Master", "MX Anywhere", "MX Vertical"][self.layout])
                .show_ui(ui, |ui| {
                    for (index, label) in ["MX Master", "MX Anywhere", "MX Vertical"]
                        .into_iter()
                        .enumerate()
                    {
                        ui.selectable_value(&mut self.layout, index, label);
                    }
                });
        });
        ...
        if self.layout == 0 {
            for (index, x, y) in [
                ...
                let point = response.rect.min
                    + Vec2::new(x * response.rect.width(), y * response.rect.height());
                let rect = egui::Rect::from_center_size(point, Vec2::splat(26.0));
                let response = ui
                    .interact(rect, ui.id().with(("mouse", index)), Sense::click())
                    .on_hover_text(LABELS[index]);
                ui.painter().circle_stroke(
                    point,
                    if self.editor.selected_button == index {
                        12.0
                    } else {
                        9.0
                    },
                ...
                if response.clicked() {
                    self.editor.selected_button = index;
                    self.sync_custom();
                }
            }
        }
        ui.horizontal_wrapped(|ui| {
            for (index, label) in LABELS.iter().enumerate() {
                if ui
                    .selectable_label(self.editor.selected_button == index, *label)
                    .clicked()
                {
                    self.editor.selected_button = index;
                    self.sync_custom();
        ...
        let mut action = self.editor.draft["profiles"][&self.editor.selected_profile]["mappings"]
            [key]
            .as_str()
            .unwrap_or("none")
            .to_owned();
        ...
        let label = ACTIONS
            .iter()
            .find(|(id, _)| *id == action)
            .map_or(action.as_str(), |(_, label)| *label)
            .to_owned();
        ...
        egui::ComboBox::from_id_salt("action")
            ...
            .show_ui(ui, |ui| {
                for &(id, label) in ACTIONS {
                    ui.selectable_value(&mut action, id.into(), label);
                }
            });
        if action != before {
            if let Err(error) = self.editor.set_action(index, &action) {
        ...
        ui.horizontal(|ui| {
            ...
            if ui.button("Set shortcut").clicked() {
                if let Err(error) = self
                    .editor
                    .set_action(index, &format!("custom:{}", self.custom.trim()))
                {
                    self.message = Some(error);
                }
            }
    ...
    fn settings(&mut self, ui: &mut egui::Ui) {
        let bounds = self
            ...
            .map_or((200, 8000), |device| {
                (device.dpi_min.max(200), device.dpi_max.max(200))
            });
        ...
        integer(
            ...
            "dpi",
            "Pointer DPI",
            i64::from(bounds.0)..=i64::from(bounds.1.max(bounds.0)),
        );
        ui.horizontal(|ui| {
                ...
                match values {
                    Ok(values)
                        if !values.is_empty()
                            ...
                            && values
                                .iter()
                                .all(|value| (bounds.0..=bounds.1).contains(value)) =>
                    {
                        settings["dpi_presets"] = json!(values)
                    }
                    _ => {
                        self.message = Some(format!(
                            "Enter 1 to 64 comma-separated DPI values between {} and {}",
                            bounds.0, bounds.1
                        ))
                    }
                }
        ...
        let mut mode = settings["smart_shift_mode"]
            .as_str()
            .unwrap_or("ratchet")
            .to_owned();
        ui.horizontal(|ui| {
        ...
        checkbox(
            ...
            settings,
            "ignore_trackpad",
            "Ignore trackpad and continuous scrolling on macOS",
        );
        integer(
            ui,
            settings,
            "hscroll_threshold",
            "Horizontal action threshold",
            1..=100,
        );
        ui.separator();
        integer(
    ...
    fn profiles(&mut self, ui: &mut egui::Ui) {
        ...
        let mut label = self.editor.draft["profiles"][&key]["label"]
            .as_str()
            .unwrap_or(&key)
            .to_owned();
        ui.horizontal(|ui| {
            ...
            if ui.text_edit_singleline(&mut label).changed() {
                self.editor.draft["profiles"][&key]["label"] = json!(label);
            }
        });
        if key == "default" {
            ui.label("This profile applies when no application-specific profile matches.");
        } else {
            ...
            if ui
                ...
                .changed()
            {
                if let Err(error) = self.editor.set_applications(&self.apps) {
                    self.message = Some(error);
                }
            }
            if ui.button("Add current foreground application").clicked() {
                if let Some(state) = &self.editor.latest {
                    let name = if !state.foreground.id.is_empty() {
                        state.foreground.id.clone()
                    } else {
                        state.foreground.name.clone()
                    };
                    if !name.is_empty() {
            ...
            if self.delete_profile {
                ui.horizontal(|ui| {
                    ...
                    if ui.button("Remove").clicked() {
                        if let Err(error) = self.editor.remove_profile(&key) {
                            self.message = Some(error);
                        }
                        self.delete_profile = false;
                        self.sync_fields();
    ...
    fn diagnostics(&mut self, ui: &mut egui::Ui) {
        if let Some(state) = &self.editor.latest {
            ...
            ui.label(format!(
                ...
                state.hardware_pending
            ));
            ui.label(format!("Dropped queue events: {}", state.dropped_events));
            if let Some(device) = &state.device {
                ui.label(format!(
                    "Backend: {} | Receiver slot: {} | Raw XY: {}",
                    device.backend, device.receiver_slot, device.raw_xy
                ));
            }
        ...
        ui.horizontal(|ui| {
            ...
            if ui
                .add_enabled(self.connected, egui::Button::new("Reconnect mouse"))
                .clicked()
            {
                self.send(Request::Reconnect, false);
            }
            if ui
                .add_enabled(self.connected, egui::Button::new("Read hardware settings"))
                .clicked()
            {
                self.send(Request::RefreshHardware, false);
            }
            if ui
                .add_enabled(self.connected, egui::Button::new("Input permissions"))
                .clicked()
            {
                self.send(Request::RequestPermissions, false);
            }
        });
    }
    ...
    fn bottom(&mut self, ui: &mut egui::Ui) {
        ui.separator();
        if self.editor.conflicted() {
            ui.label("Settings changed in another session. Your draft is preserved. Reload before saving.");
        }
        ...
        ui.horizontal(|ui| {
            let save =
                self.connected && self.editor.dirty() && !self.editor.conflicted() && !self.busy;
            if ui.add_enabled(save, egui::Button::new("Save")).clicked() {
                match self.editor.apply_request() {
                    Ok(request) => self.send(request, true),
                    Err(error) => self.message = Some(error),
                }
            }
            if ui
                .add_enabled(
                    !self.busy && self.editor.latest.is_some(),
            ...
            if ui
                ...
                .clicked()
            {
                self.send(Request::SetPaused { value: !paused }, false);
            }
            if self.editor.dirty() {
        ...
        if self.reload_confirmation {
            ui.horizontal(|ui| {
                ...
                if ui.button("Discard draft").clicked() {
                    if let Err(error) = self.editor.reload() {
                        self.message = Some(error);
                    }
    ...
    fn ui(&mut self, ui: &mut egui::Ui, _frame: &mut eframe::Frame) {
        ...
        egui::CentralPanel::default().show(ui, |ui| {
            ...
            egui::ScrollArea::vertical()
                .max_height((ui.available_height() - 125.0).max(150.0))
                .show(ui, |ui| {
                    ui.add_enabled_ui(!self.busy, |ui| match self.page {
                        0 => self.mouse_view(ui),
                        1 => self.settings(ui),
                        2 => self.profiles(ui),
                        _ => self.diagnostics(ui),
                    });
                });
            self.bottom(ui);
        });
...
fn main() -> eframe::Result {
    ...
    for argument in std::env::args().skip(1) {
        match argument.as_str() {
            ...
            "--help" | "-h" => {
                println!(
                    "SuperLight settings\n--demo         Preview without connecting to the service\n--smoke-test   Render four preview frames and exit"
                );
                return Ok(());
            }
            _ => {
    ...
    eframe::run_native(
        "SuperLight settings",
        options,
        Box::new(move |context| Ok(Box::new(Settings::new(context, demo, smoke)?))),
    )
}
