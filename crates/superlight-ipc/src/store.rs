    ...
    pub fn apply(&mut self, expected_revision: u64, value: Value) -> io::Result<()> {
        if expected_revision != self.revision {
            return Err(io::Error::new(
                ...
                "Settings changed in another operation. Reload before saving.",
            ));
        }
...
pub fn read_limited(path: &Path, limit: usize) -> io::Result<Vec<u8>> {
    ...
    if bytes.len() > limit {
        return Err(invalid("File exceeds the configured size limit"));
    }
    Ok(bytes)
}
...
pub fn atomic_write(path: &Path, bytes: &[u8]) -> io::Result<bool> {
        ...
        && (!metadata.is_file() || metadata.file_type().is_symlink())
    {
        return Err(io::Error::new(
            ...
            "Refusing to replace a symbolic link or non-file",
        ));
    }
    let parent = path
        .parent()
...
mod tests {
    ...
    fn single_instance_lock_is_released_by_drop() {
        ...
        assert_eq!(
            Store::open(paths.clone()).err().unwrap().kind(),
            io::ErrorKind::AlreadyExists
        );
        drop(store);
    ...
    fn configuration_changes_are_atomic_and_revision_checked() {
        ...
        assert_eq!(
            config::parse(&fs::read(&store.paths.config).unwrap()).unwrap()["settings"]["dpi"],
            1600
        );
    }
