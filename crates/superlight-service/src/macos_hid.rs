...
impl Manager {
    fn new(candidate: Option<&Candidate>) -> io::Result<Self> {
        ...
        if let Some(candidate) = candidate {
            ...
            if candidate.usage_page != 0 {
                properties.push(("PrimaryUsagePage", i32::from(candidate.usage_page)));
            }
            if candidate.usage != 0 {
                properties.push(("PrimaryUsage", i32::from(candidate.usage)));
            }
        }
        for (key, value) in properties {
            ...
            unsafe {
                CFDictionarySetValue(matching.0, key.0, value.0);
            }
        }
        unsafe {
        ...
        if result != 0 {
            return Err(os_error("Open Logitech IOHID manager", result));
        }
        Ok(Self(manager))
    }
    ...
    fn devices(&self) -> io::Result<Vec<Owned>> {
        ...
        if !(0..=256).contains(&count) {
            return Err(io::Error::other(
                "The Logitech device inventory exceeds its safety limit",
            ));
        }
        let mut pointers = vec![ptr::null(); count as usize];
        unsafe {
            CFSetGetValues(set.0, pointers.as_mut_ptr());
        }
        pointers
            .into_iter()
            .map(|device| unsafe { Owned::retained(device) })
            .collect()
    }
}
...
fn identity(device: Cf) -> io::Result<u64> {
    ...
    if service == 0 {
        return Err(io::Error::other(
            "The Logitech device has no registry identity",
        ));
    }
    let mut identity = 0;
    let result = unsafe { IORegistryEntryGetRegistryEntryID(service, &mut identity) };
    if result != 0 || identity == 0 {
        return Err(os_error("Read Logitech registry identity", result));
    }
    Ok(identity)
}
...
pub(crate) fn enumerate() -> io::Result<Vec<Candidate>> {
    ...
    for device in manager.devices()? {
        ...
        if !(0..=65535).contains(&product)
            || !(0..=65535).contains(&page)
            || !(0..=65535).contains(&usage)
            || !devices::candidate_allowed(vendor as u16, product as u16, &name, page as u16)
        {
            continue;
        }
        let Ok(identifier) = identity(device.0) else {
            continue;
        };
    ...
    fn read(&mut self, buffer: &mut [u8; 64]) -> io::Result<Option<usize>> {
        if self.removed {
            return Err(io::Error::new(
                ...
                "The Logitech IOHID device disconnected",
            ));
        }
        if self.overflow {
            return Err(io::Error::other(
                "The bounded Logitech report queue overflowed. Reconnecting to release captured input.",
            ));
        }
        if let Some(fault) = self.fault {
            return Err(os_error("Read Logitech IOHID report", fault));
        }
...
unsafe extern "C" fn input_callback(
    ...
    let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        if !(1..=64).contains(&length) || bytes.is_null() {
            if report_type == 0 && matches!(report_id, 0x10 | 0x11) {
                unsafe {
                    (*state).overflow = true;
                }
            }
...
unsafe extern "C" fn removal_callback(context: *mut c_void, _: i32, _: Cf) {
    if !context.is_null() {
        unsafe {
            (*context.cast::<CallbackState>()).removed = true;
        }
    }
}
...
impl Device {
    pub(crate) fn open(candidate: &Candidate) -> io::Result<Self> {
        let expected = candidate
            ...
            .to_str()
            .ok()
            .and_then(|path| path.strip_prefix("iokit:"))
            .and_then(|identifier| identifier.parse::<u64>().ok())
            .ok_or_else(|| io::Error::other("Invalid Logitech IOHID identity"))?;
        let manager = Manager::new(Some(candidate))?;
        let device = manager
            .devices()?
            .into_iter()
            .find(|device| identity(device.0).ok() == Some(expected))
            .ok_or_else(|| {
                io::Error::new(
                    io::ErrorKind::NotFound,
                    "The selected Logitech IOHID interface is no longer present",
                )
            })?;
        ...
        if result != 0 {
            return Err(os_error("Open shared Logitech IOHID device", result));
        }
        unsafe {
    ...
    fn thread_check(&self) -> io::Result<()> {
        if thread::current().id() != self.owner {
            return Err(io::Error::other(
                "The Logitech IOHID handle must stay on its owning thread",
            ));
        }
        Ok(())
    }
    ...
    pub(crate) fn write(&mut self, report: &[u8; 20]) -> io::Result<usize> {
        ...
        if result != 0 {
            return Err(os_error("Write Logitech IOHID report", result));
        }
        Ok(report.len())
    }
    ...
    pub(crate) fn read(&mut self, buffer: &mut [u8; 64], timeout: Duration) -> io::Result<usize> {
        ...
        loop {
            if let Some(length) = unsafe { (&mut *self.state.get()).read(buffer)? } {
                return Ok(length);
            }
            ...
            if matches!(result, 1 | 2) {
                return Err(io::Error::new(
                    io::ErrorKind::NotConnected,
                    "The Logitech IOHID run loop stopped",
                ));
            }
        }
    }
