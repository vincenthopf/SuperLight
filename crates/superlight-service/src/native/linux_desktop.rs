...
impl Desktop {
    fn open() -> Result<Self> {
        ...
        if !display.starts_with(':') && !display.starts_with("unix:") {
            return Err(io::Error::new(
                ...
                "Only local X11 desktop connections are used",
            )
            .into());
        }
    ...
    fn foreground(&self) -> Result<AppInfo> {
        let Some(window) = self
            .integer(self.root, self.active, AtomEnum::WINDOW)?
            .filter(|window| *window != 0)
        else {
            return Ok(AppInfo::default());
        };
        let pid = self
            .integer(window, self.pid, AtomEnum::CARDINAL)?
            .unwrap_or(0);
        let reply = self
            .connection
            .get_property(false, window, AtomEnum::WM_CLASS, AtomEnum::STRING, 0, 1024)?
            .reply()?;
        let mut aliases: Vec<String> = reply
            .value
            .split(|byte| *byte == 0)
            .filter(|part| !part.is_empty())
            .take(2)
            .map(|part| String::from_utf8_lossy(part).chars().take(255).collect())
            .collect();
    ...
    fn focus(&self, pid: u32) -> Result<()> {
        let reply = self
            .connection
            .get_property(false, self.root, self.clients, AtomEnum::WINDOW, 0, 1024)?
            .reply()?;
        ...
        Err(io::Error::new(
            ...
            "The settings window is not visible to this desktop",
        )
        .into())
    }
}
...
pub fn focus(pid: u32) -> io::Result<()> {
    let mut desktop = DESKTOP
        .lock()
        .map_err(|_| io::Error::other("The desktop connection is unavailable"))?;
    if desktop.is_none() {
        *desktop = Some(Desktop::open().map_err(io::Error::other)?);
    }
    desktop
        .as_ref()
        .ok_or_else(|| io::Error::other("No local X11 desktop is available"))?
        .focus(pid)
