use super::{
    UiEvent, linux_desktop,
    linux_frame::{self, Frame},
    login,
};
use crate::{
    hook::Hook,
...
pub fn error_dialog(message: &str) {
    eprintln!("SuperLight: {message}");
}
pub fn foreground() -> AppInfo {
    linux_desktop::foreground()
}

fn emit(events: &[InputEvent]) -> io::Result<()> {
    let mut output = OUTPUT
        .lock()
        .map_err(|_| io::Error::other("The virtual input device is unavailable"))?;
    output
        .as_mut()
        .ok_or_else(|| {
            io::Error::new(
                io::ErrorKind::PermissionDenied,
                "Allow access to /dev/uinput before enabling remapping",
            )
        })?
        .emit(events)
}
...
fn mouse_code(button: u8) -> io::Result<KeyCode> {
    ...
    .copied()
    .ok_or_else(|| io::Error::other("Invalid mouse button"))
}
...
pub fn chord(chord: Chord) -> io::Result<()> {
    ...
    if let Err(error) = emit(&events[..keys.len() * 2]) {
        for (index, &key) in keys.iter().rev().enumerate() {
            events[index] = InputEvent::new(EventType::KEY.0, key, 0);
        }
...
fn ensure_output() -> io::Result<()> {
    ...
    *output = Some(
        VirtualDevice::builder()?
            .name("SuperLight configured actions")
            .with_keys(&keys)?
...
pub fn mouse_matches(status: &DeviceStatus, vendor: u16, product: u16, name: &str) -> bool {
    if vendor != hidpp::VENDOR || name.starts_with("SuperLight") {
        return false;
    }
    if normalized(name) == normalized(&status.name) && !name.is_empty() {
        return true;
    }
    if let Some(spec) = devices::resolve(product, name)
        && spec.key == status.model_key
    {
        return true;
    }
    status.receiver_slot == 0xff
        && (0xb000..=0xbfff).contains(&status.product_id)
...
fn choose(status: &DeviceStatus) -> io::Result<Option<Device>> {
    ...
    for (_, device) in evdev::enumerate().take(128) {
        ...
        let mouse = device.supported_keys().is_some_and(|keys| {
        ...
        if result.is_some() {
            return Err(io::Error::other(
                "More than one evdev mouse matches the selected HID device. No device was grabbed. Disconnect the duplicate mouse before reconnecting.",
            ));
        }
        result = Some(device);
    }
...
fn identity(status: &DeviceStatus) -> String {
    format!(
        "{:04x}/{}/{}",
        status.product_id, status.receiver_slot, status.name
    )
}
...
impl MouseInput {
    fn open(
        ...
        let keys = device
            .supported_keys()
            .ok_or_else(|| io::Error::other("The selected mouse has no button capabilities"))?;
        let axes = device
            .supported_relative_axes()
            .ok_or_else(|| io::Error::other("The selected mouse has no relative axes"))?;
        let mirror = VirtualDevice::builder()?
            .name("SuperLight forwarded mouse")
            .with_keys(keys)?
            .with_relative_axes(axes)?
            .with_properties(device.properties())?
            .build()?;
        std::thread::sleep(Duration::from_millis(250));
        if shared.stopping() || device.get_key_state()?.iter().next().is_some() {
            return Ok(None);
        }
        ...
        Ok(Some(Self {
            device,
            mirror,
            hook: Hook::new(Arc::clone(shared)),
            frame: Frame::default(),
            filtered: Frame::default(),
            forwarded: [false; 768],
            ...
            Ok(events) => {
                for event in events {
                    if event.event_type() == EventType::SYNCHRONIZATION {
                        if event.code() == SynchronizationCode::SYN_DROPPED.0 {
                            return Err(io::Error::other(
                                "The mouse event stream lost synchronization. Native input is being restored.",
                            ));
                        }
                            ...
                            if !filtered.events().is_empty() {
                                ...
                                for event in filtered.events() {
                                    if event.event_type() == EventType::KEY
                                        && let Some(held) =
                                            forwarded.get_mut(usize::from(event.code()))
                                    {
                                        *held = event.value() != 0;
                                    }
                                }
    ...
    fn drop(&mut self) {
        for (code, held) in self.forwarded.iter().enumerate() {
            if *held {
                let _ = self
                    .mirror
                    .emit(&[InputEvent::new(EventType::KEY.0, code as u16, 0)]);
            }
        }
...
fn boot_bias() -> Option<f64> {
    ...
    if unsafe { libc::clock_gettime(libc::CLOCK_BOOTTIME, &mut boot) } != 0
        || unsafe { libc::clock_gettime(libc::CLOCK_MONOTONIC, &mut awake) } != 0
    {
        return None;
    }
    Some(
        (boot.tv_sec - awake.tv_sec) as f64
            + (boot.tv_nsec - awake.tv_nsec) as f64 / 1_000_000_000.0,
    )
}
...
pub fn post(event: UiEvent) {
    let flags = match event {
        ...
        UiEvent::Refresh => 4,
        UiEvent::Focus(pid) => {
            if let Err(error) = linux_desktop::focus(pid)
...
pub fn set_start_at_login(enabled: bool, executable: &Path) -> io::Result<()> {
    ...
    let root = std::env::var_os("XDG_CONFIG_HOME")
        ...
        .ok_or_else(|| io::Error::other("The user configuration directory is unavailable"))?;
...
fn publish(
    ...
    inject: bool,
    description: &str,
    previous: &mut Option<(bool, Permissions)>,
) {
    if !ready && shared.native_ready.swap(false, Ordering::AcqRel) {
        shared.release_all();
    }
    if previous.as_ref().is_some_and(|(old_ready, permissions)| {
        *old_ready == ready
            && permissions.inject == inject
...
pub fn run(shared: Arc<Shared>) -> io::Result<()> {
    ...
    WAKE.set(unsafe { OwnedFd::from_raw_fd(descriptor) })
        .map_err(|_| io::Error::other("The native event loop is already running"))?;
    let wayland =
        std::env::var("XDG_SESSION_TYPE").is_ok_and(|value| value.eq_ignore_ascii_case("wayland"));
    let ready_description = if wayland {
        "The selected Logitech mouse is available through evdev/uinput. Native Wayland applications use the default profile because this desktop does not expose a portable foreground-application API."
    } else {
        "The selected Logitech mouse is available through evdev/uinput. Other input devices are unchanged."
    };
    ...
    while !shared.stopping() {
        ...
        if events & 4 != 0 || Instant::now() >= next_status {
            ...
            if input.as_ref().is_some_and(|input| {
                status
                    .as_ref()
                    .is_none_or(|status| input.identity != identity(status))
            }) {
                input = None;
                shared.release_all();
            }
        }
        if (!shared.device_connected.load(Ordering::Acquire)
            || shared.suspended.load(Ordering::Acquire))
            && input.take_if(|_| true).is_some()
        {
            shared.release_all();
        }
        if input.is_none() && shared.native_ready.load(Ordering::Acquire) {
            publish(
                ...
                false,
                injection_ready,
                "Waiting for the verified Logitech mouse connection",
                &mut published,
            );
        }
        if input.is_none() && Instant::now() >= retry && !shared.suspended.load(Ordering::Acquire) {
            ...
            injection_ready = match ensure_output() {
                ...
                Err(error) => {
                    publish(
                        ...
                        false,
                        &format!(
                            "Cannot open /dev/uinput: {error}. Install the supplied user-access udev rule before enabling remapping."
                        ),
                        &mut published,
                    );
                    false
            ...
            if injection_ready
                && shared.device_connected.load(Ordering::Acquire)
                && let Some(status) = &status
            {
                match choose(status).and_then(|device| match device {
                    Some(device) => MouseInput::open(device, status, &shared),
                    None => Ok(None),
                }) {
                    ...
                    Ok(None) => publish(
                        ...
                        false,
                        true,
                        "No accessible evdev mouse matches the selected Logitech HID device, or a button is still held. Release all buttons and check the supplied udev rules.",
                        &mut published,
                    ),
                ...
                publish(
                    ...
                    false,
                    true,
                    "Waiting for a verified Logitech HID++ mouse. Other input devices have not been grabbed.",
                    &mut published,
                );
            }
        }
        ...
        let mut descriptors = [
            ...
            libc::pollfd {
                fd: input.as_ref().map_or(-1, |input| input.device.as_raw_fd()),
                events: libc::POLLIN,
                revents: 0,
            },
        ];
        ...
        if let Some(error) = failure {
            ...
            publish(
                ...
                false,
                injection_ready,
                &format!("Native input was restored: {error}"),
                &mut published,
            );
            retry = Instant::now() + RETRY;
        }
...
mod tests {
    ...
    fn desktop_entry_paths_are_quoted_without_field_expansion() {
        assert_eq!(
            desktop_exec("/home/me/My App/superlight"),
            "\"/home/me/My App/superlight\""
        );
        assert_eq!(
            desktop_exec("/home/me/100%/superlight"),
            "\"/home/me/100%%/superlight\""
        );
