...
use superlight_core::{
    actions::Chord,
    input::{INJECTION_MARKER, INVERT_MARKER},
};
...
pub fn mouse(button: u8, down: bool) -> io::Result<()> {
    if button > 4 {
        return Err(io::Error::other("Invalid mouse button"));
    }
    ...
    let event = Owned::new(unsafe {
        CGEventCreateMouseEvent(ptr::null(), kind, point, u32::from(button))
    })?;
...
pub fn chord(chord: Chord) -> io::Result<()> {
    ...
    let flags = chord
        .keys()
        .iter()
        .fold(0, |flags, key| flags | modifier(*key));
    for &key in chord.keys() {
        ...
        down.push(event);
        up.push(Owned::new(unsafe {
            CGEventCreateKeyboardEvent(ptr::null(), key, 0)
        })?);
    }
    for event in &down {
        mark_and_post(event.0);
...
pub fn scroll(horizontal: bool, delta: i32) -> io::Result<()> {
    let event = Owned::new(unsafe {
        if horizontal {
            CGEventCreateScrollWheelEvent(ptr::null(), 0, 2, 0i32, delta)
        } else {
            CGEventCreateScrollWheelEvent(ptr::null(), 0, 1, delta)
        }
    })?;
    mark_and_post(event.0);
    Ok(())
...
pub fn media(key: u8) -> io::Result<()> {
    ...
    unsafe {
        for state in [0xaisize, 0xb] {
            ...
            if event.is_null() {
                return Err(io::Error::other("Could not create a native media event"));
            }
            let cg: Cf = msg0(event, c"CGEvent");
            if cg.is_null() {
                return Err(io::Error::other("Could not convert the native media event"));
            }
            mark_and_post(cg);
        }
    }
...
unsafe fn symbolic_hotkey(hotkey: u32) -> io::Result<bool> {
    ...
    if get.is_null() || enabled.is_null() || set.is_null() {
        return Ok(false);
    }
    let get: unsafe extern "C" fn(u32, *mut u16, *mut u16, *mut u32) -> i32 =
    ...
    if get(hotkey, &mut equivalent, &mut key, &mut flags) != 0 || key == u16::MAX {
        return Ok(false);
    }
    ...
    if !was_enabled && set(hotkey, true) != 0 {
        return Ok(false);
    }
    ...
    if !was_enabled {
        std::thread::sleep(Duration::from_millis(50));
        if set(hotkey, false) != 0 {
            return Err(io::Error::other(
                "The desktop shortcut ran, but macOS could not restore its previous disabled state",
            ));
        }
    }
    Ok(true)
...
pub fn system(action: u8) -> io::Result<()> {
    ...
    let notification = match action {
        0 => Some("com.apple.expose.awake"),
        1 => Some("com.apple.expose.front.awake"),
        2 => Some("com.apple.showdesktop.awake"),
        3 => Some("com.apple.launchpad.toggle"),
        _ => None,
    };
    if notification.is_some_and(|name| unsafe { dock_notification(name) }) {
        return Ok(());
    }
    if matches!(action, 4 | 5) && unsafe { symbolic_hotkey(if action == 4 { 79 } else { 81 })? } {
        return Ok(());
    }
...
pub fn foreground() -> AppInfo {
    ...
    unsafe {
        ...
        let path: Id = if executable.is_null() {
            ptr::null_mut()
        } else {
            msg0(executable, c"path")
        };
        ...
        AppInfo {
            ...
            id,
            aliases,
            input_restricted: IsSecureEventInputEnabled(),
        }
    }
}
...
pub fn set_start_at_login(enabled: bool, executable: &Path) -> io::Result<()> {
    ...
    let home =
        std::env::var_os("HOME").ok_or_else(|| io::Error::other("HOME is not configured"))?;
    ...
    if enabled {
        ...
        let plist = format!(
            "<?xml version=\"1.0\" encoding=\"UTF-8\"?><!DOCTYPE plist PUBLIC \"-//Apple//DTD PLIST 1.0//EN\" \"http://www.apple.com/DTDs/PropertyList-1.0.dtd\"><plist version=\"1.0\"><dict><key>Label</key><string>io.github.vincenthopf.SuperLight</string><key>ProgramArguments</key><array><string>{}</string><string>--background</string></array><key>RunAtLoad</key><true/><key>ProcessType</key><string>Interactive</string></dict></plist>",
            login::xml(executable)
        );
...
pub fn post(event: UiEvent) {
    ...
    if let Some((source, loop_ref)) = *LOOP_SOURCE
        .lock()
        .unwrap_or_else(|poisoned| poisoned.into_inner())
    {
        unsafe {
            CFRunLoopSourceSignal(source as Cf);
            CFRunLoopWakeUp(loop_ref as Cf);
        }
    }
}
...
unsafe extern "C" fn perform(_: *mut c_void) {
    let result = std::panic::catch_unwind(|| {
        ...
        NATIVE.with(|native| {
            if let Ok(mut native) = native.try_borrow_mut()
                && let Some(native) = native.as_mut()
            {
                ...
                if events & 8 != 0 {
                    let app: Id = msg1(
                        ...
                        FOCUS_PID.load(Ordering::Acquire) as i32,
                    );
                    if !app.is_null() {
                        let _: bool = msg1(app, c"activateWithOptions:", 2usize);
                    }
                }
                if events & 1 != 0 {
                    ...
                    if !event.is_null() {
                        msg2::<_, _, ()>(native.app, c"postEvent:atStart:", event, true);
                    }
                }
            }
        });
...
unsafe extern "C" fn settings(_: Id, _: Sel, _: Id) {
    issue(Request::ShowSettings);
}
unsafe extern "C" fn reconnect(_: Id, _: Sel, _: Id) {
    issue(Request::Reconnect);
}
unsafe extern "C" fn permissions(_: Id, _: Sel, _: Id) {
    post(UiEvent::Permissions);
}
unsafe extern "C" fn quit(_: Id, _: Sel, _: Id) {
    if let Some(shared) = SHARED.get() {
        shared.stop();
    }
}
unsafe extern "C" fn pause(_: Id, _: Sel, _: Id) {
    if let Some(shared) = SHARED.get() {
        issue(Request::SetPaused {
            value: !shared.policy.load().paused,
        });
    }
}
unsafe extern "C" fn foreground_changed(_: Id, _: Sel, _: Id) {
    if let Some(shared) = SHARED.get() {
        shared.command(Command::ForegroundChanged);
    }
}
...
unsafe fn process_event(kind: u32, event: Cf) -> Cf {
    ...
    if kind == 0xffff_fffe || kind == 0xffff_ffff {
        shared.release_all();
        TAP.with(|tap| {
            if !tap.get().is_null() && !shared.stopping() {
                CGEventTapEnable(tap.get(), 1);
            }
        });
        return event;
    }
    ...
    if marker == INJECTION_MARKER || marker == INVERT_MARKER {
        return event;
    }
    HOOK.with(|hook| {
        ...
        match kind {
            25 | 26 => {
                if let Some(source) = button_source(CGEventGetIntegerValueField(event, 3))
                    && hook.button(source, kind == 25)
                {
                    return ptr::null();
                }
            }
            5 | 6 | 7 | 27 => {
                if hook.movement(
                    CGEventGetIntegerValueField(event, 4) as f64,
                    CGEventGetIntegerValueField(event, 5) as f64,
                ) {
                    return ptr::null();
                }
                if kind == 27
                    && let Some(source) = button_source(CGEventGetIntegerValueField(event, 3))
            ...
            22 if shared.allowed() => {
                ...
                if policy.ignore_trackpad && CGEventGetIntegerValueField(event, 88) != 0 {
                    return event;
                }
                let horizontal = CGEventGetDoubleValueField(event, 94);
                let horizontal = if horizontal != 0.0 {
                    horizontal
                } else {
                    CGEventGetIntegerValueField(event, 12) as f64
                };
                if horizontal != 0.0 && hook.wheel(if horizontal > 0.0 { 5 } else { 4 }, horizontal)
                {
                    return ptr::null();
                }
                let mut changed = false;
                for (invert, integer, fixed, point) in [
                    (policy.invert_vertical, 11, 93, 96),
                    (policy.invert_horizontal, 12, 94, 97),
                ] {
                    if invert {
                        CGEventSetIntegerValueField(
                            event,
                            integer,
                            CGEventGetIntegerValueField(event, integer).saturating_neg(),
                        );
                        CGEventSetDoubleValueField(
                            event,
                            fixed,
                            -CGEventGetDoubleValueField(event, fixed),
                        );
                        CGEventSetIntegerValueField(
                            event,
                            point,
                            CGEventGetIntegerValueField(event, point).saturating_neg(),
                        );
                        changed = true;
                    }
                }
                if changed {
                    CGEventSetIntegerValueField(event, 42, INVERT_MARKER);
                }
            }
            _ => {}
        }
...
impl Native {
    unsafe fn new() -> io::Result<Self> {
        ...
        let delegate_class = register_class(
            c"SuperLightNativeCallbacks",
            &[
                (c"settings:", settings as *const c_void),
                (c"pause:", pause as *const c_void),
                (c"reconnect:", reconnect as *const c_void),
                (c"permissions:", permissions as *const c_void),
                (c"quit:", quit as *const c_void),
                (c"foreground:", foreground_changed as *const c_void),
                (c"sleeping:", sleeping as *const c_void),
                (c"awake:", awake as *const c_void),
            ],
        )?;
        ...
        for (title, action) in [
            ...
            let key = string("")?;
            let item: Id = msg3(
                msg0::<Id>(class(c"NSMenuItem"), c"alloc"),
        ...
        *LOOP_SOURCE
            .lock()
            .unwrap_or_else(|poisoned| poisoned.into_inner()) =
            Some((source.0 as usize, loop_ref as usize));
        Ok(Self {
            app,
            delegate,
            status,
    ...
    fn refresh_permissions(&mut self) {
        ...
        if listen && inject && self.tap.is_none() && !shared.stopping() {
            unsafe {
                ...
                if let Ok(tap) = Owned::new(CGEventTapCreate(
                    0,
                    0,
                    0,
                    mask,
                    ...
                    if let Ok(source) =
                        Owned::new(CFMachPortCreateRunLoopSource(ptr::null(), tap.0, 0))
                    {
        ...
        let permissions = Permissions {
            ...
            description: if ready {
                "Accessibility and Input Monitoring are available".into()
            } else {
        ...
        *LOOP_SOURCE
        ...
        unsafe {
            ...
            if let Some(tap) = &self.tap {
                CGEventTapEnable(tap.0, 0);
                CFMachPortInvalidate(tap.0);
            }
            if let Some(source) = &self.tap_source {
                CFRunLoopRemoveSource(CFRunLoopGetMain(), source.0, kCFRunLoopCommonModes);
            }
...
pub fn run(shared: Arc<Shared>) -> io::Result<()> {
    ...
    SHARED
        ...
        .map_err(|_| io::Error::other("The native event loop has already been initialized"))?;
    HOOK.with(|hook| {
        *hook.borrow_mut() = Some(Hook::new(Arc::clone(&shared)));
    });
