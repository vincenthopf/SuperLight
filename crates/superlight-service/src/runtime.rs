        ...
        let handle = thread::Builder::new().name(name.into()).spawn(move || {
            ...
            if result.is_err() {
                shared.report(format!(
                    "{name} stopped unexpectedly. Input remapping has been disabled."
                ));
                shared.stop();
            }
        })?;
...
pub fn run(options: Options) -> io::Result<()> {
    ...
    let show_ui = !options.headless
        && !options.background
        && (options.force_ui
            || store.first_run
            || !policy::boolean(&store.value, "start_minimized", true));
    let server = Server::bind(paths)?;
    let (shared, inputs, commands) = Shared::new(
        store.value.clone(),
        server.endpoint.instance.clone(),
        options.headless,
    )
    ...
    workers.spawn("superlight-output", move || {
        output::run(output_shared, inputs)
    })?;
    workers.spawn("superlight-controller", move || {
        controller.run(commands, show_ui)
    })?;
    if !options.headless {
...
fn serve(server: Server, shared: Arc<Shared>) {
    while !shared.stopping() {
        ...
        let response = if matches!(request, Request::Get) {
            ...
            if shared
                .commands
                .send_timeout(Command::Rpc { request, reply }, Duration::from_millis(250))
                .is_err()
            {
...
pub fn apply_settings(
    ...
    value: Value,
    mut set_login: impl FnMut(bool) -> io::Result<()>,
) -> io::Result<()> {
    ...
    if let Err(error) = store.apply(expected_revision, value) {
        if old_login != new_login
            && let Err(rollback) = set_login(old_login)
        {
            return Err(io::Error::other(format!(
                "{error}. Restoring the previous login setting also failed: {rollback}"
            )));
        }
        return Err(error);
    }
...
pub fn device_action(
    ...
    match action {
        Action::ToggleSmartShift => {
            value["settings"]["smart_shift_enabled"] =
                json!(!policy::boolean(&value, "smart_shift_enabled", false));
        }
        ...
        Action::CycleDpi => {
            ...
            for dpi in candidates.iter().take(64).filter_map(Value::as_i64) {
                ...
                if !presets.contains(&dpi) {
                    presets.push(dpi);
                }
            }
            if presets.is_empty() {
                return Err("DPI presets must contain at least one number".into());
            }
            let current = policy::number(&value, "dpi", 1000.0) as u16;
            let index = presets
                .iter()
                .position(|dpi| *dpi == current)
...
impl Controller {
    ...
    fn run(mut self, receiver: Receiver<Command>, show_ui: bool) {
        ...
        if show_ui && let Err(error) = self.show_settings() {
            self.record_error(error.to_string());
        }
        let mut last_poll = Instant::now();
        while !self.shared.stopping() {
            let interval = if self.store.value["profiles"]
                .as_object()
            ...
            if last_poll.elapsed() >= interval {
                ...
                if !self.shared.headless {
                    native::post(native::UiEvent::Refresh);
                }
    ...
    fn refresh_foreground(&mut self) {
        ...
        if foreground != self.foreground {
            ...
            let profile =
                config::profile_for_aliases(&self.store.value, &self.foreground.aliases).to_owned();
        ...
        apply_settings(&mut self.store, revision, value, |enabled| {
            if headless {
                return Err(io::Error::new(
                    ...
                    "Login integration is disabled in headless verification mode",
                ));
            }
    ...
    fn show_settings(&mut self) -> io::Result<()> {
        if self.shared.headless {
            return Err(io::Error::new(
                io::ErrorKind::Unsupported,
                "The settings window is disabled in headless verification mode",
            ));
        }
        if let Some(child) = self.ui.as_mut() {
        ...
        if !path.is_file() {
            return Err(io::Error::new(
                ...
                "The superlight-ui executable is missing. Install the complete application bundle.",
            ));
        }
    ...
    fn request(&mut self, request: Request) -> Response {
        let result = match request {
            ...
            Request::Apply {
                expected_revision,
                config,
            } => self.apply(expected_revision, config),
            ...
            Request::RequestPermissions => {
                if self.shared.headless {
                    Err(io::Error::new(
                        io::ErrorKind::Unsupported,
                        "Native permissions are disabled in headless verification mode",
                    ))
                } else {
                    native::post(native::UiEvent::Permissions);
                    Ok(())
                }
            }
        ...
        match result {
            Ok(()) => Response::success(self.shared.status()),
    ...
    fn handle(&mut self, command: Command) {
        match command {
            Command::Rpc { request, reply } => {
                let _ = reply.try_send(self.request(request));
            }
            ...
            Command::Action(action) => {
                ...
                if let Some(device) = device {
                    let supported = if action == Action::CycleDpi {
                        device.supports_dpi
                    } else {
                        device.supports_smart_shift
                    };
                    if !supported {
                        self.record_error(
                            "The connected mouse does not support that device action".into(),
                        );
                        return;
                    }
                    match device_action(&self.store.value, action, device.dpi_min, device.dpi_max) {
                        Ok(value) => {
                            if let Err(error) = self.apply(self.store.revision, value) {
                                self.record_error(error.to_string());
                            }
                        }
