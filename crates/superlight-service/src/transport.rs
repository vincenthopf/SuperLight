...
impl Candidate {
    ...
    pub fn backend(&self) -> &'static str {
        if cfg!(target_os = "macos") && self.path.as_bytes().starts_with(b"iokit:") { "IOKit" } else { "hidapi" }
    }
}
...
pub fn enumerate(api: &mut HidApi) -> Result<Vec<Candidate>, Error> {
    ...
    #[cfg(target_os = "macos")]
    if let Ok(native) = crate::macos_hid::enumerate() { candidates.extend(native); }
    if candidates.is_empty() && let Some(error) = refresh_error { return Err(Error::Transport(error.to_string())); }
...
impl HidTransport {
    pub fn open(api: &HidApi, candidate: &Candidate, shared: &Arc<Shared>) -> Result<(Self, AccessHandle), Error> {
        ...
        if candidate.backend() == "IOKit" {
            let device = crate::macos_hid::Device::open(candidate).map_err(|error| Error::Transport(error.to_string()))?;
    ...
    fn with_device(device: Device, shared: &Arc<Shared>) -> (Self, AccessHandle) {
        ...
        (Self { device, shared: Arc::downgrade(shared), access: Arc::clone(&access) }, access)
    }
}
...
impl Transport for HidTransport {
    fn write_report(&mut self, report: &[u8; 20]) -> Result<(), Error> {
        ...
        let written = match &mut self.device {
            Device::Hidapi(device) => device.write(report).map_err(|error| Error::Transport(error.to_string()))?,
            #[cfg(target_os = "macos")]
            Device::Iokit(device) => device.write(report).map_err(|error| Error::Transport(error.to_string()))?,
        };
    ...
    fn read_wait(&mut self, buffer: &mut [u8; 64], timeout: Duration) -> Result<usize, Error> {
        ...
        let count = match &mut self.device {
            Device::Hidapi(device) => device.read_timeout(buffer, milliseconds).map_err(|error| Error::Transport(error.to_string()))?,
            #[cfg(target_os = "macos")]
            Device::Iokit(device) => device.read(buffer, Duration::from_millis(milliseconds as u64)).map_err(|error| Error::Transport(error.to_string()))?,
        };
