...
impl Shared {
    pub fn new(value: Value, instance: String, headless: bool) -> Result<SharedChannels, String> {
        ...
        let shared = Arc::new(Self {
            ...
            config: ArcSwap::from_pointee(value),
            input,
            commands,
    ...
    pub fn emit(&self, event: Input) -> bool {
        ...
        if self.input.try_send(message).is_ok() {
            return true;
        }
...
mod tests {
    ...
    fn input_queue_never_grows_and_cancellation_invalidates_old_events() {
        ...
        let event = Input::Dispatch(Dispatch {
            source: 0,
        ...
        for queued in input.try_iter() {
            assert_ne!(queued.epoch, epoch);
        }
    ...
    fn native_input_requires_connection_permissions_and_an_active_policy() {
        let (shared, _, _) =
            Shared::new(superlight_core::config::defaults(), "test".into(), true).unwrap();
