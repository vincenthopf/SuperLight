    ...
    var body: some View {
        VStack(spacing: 2) {
            ForEach(0..<7) { index in
                ...
                if filter.isEmpty || controlNames[index].localizedCaseInsensitiveContains(filter) || action.localizedCaseInsensitiveContains(filter) {
            ...
            if !filter.isEmpty && !(0..<7).contains(where: { controlNames[$0].localizedCaseInsensitiveContains(filter) || model.data.profiles[model.profile].actions[$0].localizedCaseInsensitiveContains(filter) }) {
                ContentUnavailableView.search(text: filter).frame(height: 160)
            }
        }
    }
}
...
struct ActionEditor: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 18) {
            ...
            Button("Use default behavior") { model.assign("Default behavior") }.buttonStyle(.link)
        }
...
struct ActionChooser: View {
        ...
        VStack(alignment: .leading, spacing: 16) {
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 14) {
                    ForEach(actionGroups, id: \.0) { group in
                    ...
                    if !query.isEmpty && !actionGroups.flatMap(\.1).contains(where: { $0.localizedCaseInsensitiveContains(query) }) {
            ...
            Text(validation.isEmpty ? "Use cmd, ctrl, alt or shift, followed by a key. Preview only." : validation)
    ...
    func useShortcut() {
        ...
        model.assign(parts.joined(separator: "+"))
        dismiss()
    }
}
